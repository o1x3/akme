...
if [ -f npm/package.json ]; then
  ...
  if [ "$npm_name" != "akme-cli" ]; then
    echo "npm/package.json name must be akme-cli (got ${npm_name})" >&2
    exit 1
  fi
  ...
  node -e '
    ...
    if (main.optionalDependencies && Object.keys(main.optionalDependencies).length) {
      console.error("npm/package.json must not list optionalDependencies (single-package GitHub install)");
      process.exit(1);
    }
  '
fi
