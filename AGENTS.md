...
`VERSION` is the release trigger. GoReleaser owns GitHub releases and macOS/Linux artifacts. Runtime self-update consumes the latest GitHub release asset for the current OS and architecture. The npm package `@o1x3/akme` is a second distribution channel: `.github/workflows/release.yml` publishes platform packages (`@o1x3/akme-<os>-<arch>`) that embed the matching binary, then the meta package with those listed under `optionalDependencies` (esbuild/Sentry pattern). Installs therefore pull the binary from the npm CDN; `npm/lib/binary.js` only falls back to GitHub release downloads when optional deps were omitted. GoReleaser must finish first so `dist/` archives exist for staging. CI publishes via npm **Trusted Publishing** (OIDC, `id-token: write`) — no `NPM_TOKEN`. Configure the trusted publisher on `@o1x3/akme` and each platform package (GitHub user `o1x3`, repo `akme`, workflow `release.yml`). Unscoped `akme` is blocked by npm's name-similarity rules pending support approval.
...
- `npm/package.json` `version` bumped to match `VERSION` (enforced by `scripts/validate-version.sh`, which also requires the package `name` to stay `@o1x3/akme` and `optionalDependencies` to list the four platform packages at the same version)
...
- Full local gate: `scripts/check.sh` is exactly what CI runs (`.github/workflows/ci.yml` = Go from `go.mod` + Node 22 + `scripts/check.sh`), so a green local run means green CI. It runs `gofmt -l .`, `go test ./...`, `sh -n` on the shell scripts, and version validation. It ALSO checks the npm package (`node --check` on `npm/bin`, `npm/lib`, `npm/scripts` JS + `node --test npm/lib/*.test.js`) — but only when `node` is on PATH, otherwise those checks are silently skipped. The cloud VM ships Node 22, so the full gate (Go + npm) runs here; do not rely on a machine without `node` to catch npm-package regressions.
- Version gate: `scripts/validate-version.sh` requires `VERSION` to be plain `major.minor.patch`, to have a matching `## <version>` section in `CHANGELOG.md`, and to equal `npm/package.json`'s `version` (with `name` == `@o1x3/akme` and matching `optionalDependencies` platform package versions). It shells out to `node` to read `npm/package.json`, so it needs `node` present when that file exists. Bumping `VERSION` without also updating `CHANGELOG.md` and `npm/package.json` fails the gate.
